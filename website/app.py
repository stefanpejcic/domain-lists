...
def load_keywords():
    """Tracked keywords for /trending, configured in keywords.json."""
    data = load_json(KEYWORDS_CONFIG) or {}
    return [k.strip().lower() for k in data.get("keywords", []) if k.strip()]


def latest_date_with_keywords():
    """
    Find the most recent date folder that has keyword lists (mirrors
    scripts/generate_keyword_lists.py's output layout).
    """
    if not LISTS_FOLDER.exists():
        return None
...
def latest_date_with_live():
    """
    Find the most recent date folder that has live-cert lists (mirrors
    scripts/generate_live_domains.py's output layout).
    """
    if not LISTS_FOLDER.exists():
        return None
    ...
    dates = [
        d.name for d in LISTS_FOLDER.iterdir()
        if d.is_dir() and (d / "live").is_dir()
    ]
    return sorted(dates, reverse=True)[0] if dates else None


def latest_date_with_threats():
    """
    Find the most recent date folder that has threat-match lists
    (mirrors scripts/generate_threat_matches.py's output layout).
    """
    if not LISTS_FOLDER.exists():
        return None
    ...
    dates = [
        d.name for d in LISTS_FOLDER.iterdir()
        if d.is_dir() and (d / "threats").is_dir()
    ]
    return sorted(dates, reverse=True)[0] if dates else None
...
# ==============================================================
# Trending Keyword Routes
#
# Lists are generated by scripts/generate_keyword_lists.py, filtering
# each day's new/all.txt.gz by the keywords configured in
# keywords.json. This mirrors the /deleted pattern routes above.
# ==============================================================
@app.route("/trending", defaults={"keyword": None})
...
def trending(keyword):
    ...
    if keyword:
        ...
        summary_data = load_json(JSON_FOLDER / "keywords.json") or {}
        counts = summary_data.get("keywords", {})

        file_path = LISTS_FOLDER / date / "keywords" / f"{keyword}.txt.gz"
        file_size = file_path.stat().st_size if file_path.exists() else 0
    ...
    data = load_json(JSON_FOLDER / "keywords.json")

    if not data:
        return jsonify({
            "error": "No keyword statistics found",
            "keywords": {}
        }), 404
...
@app.route("/trending/<keyword>/browse")
def browse_trending(keyword):
    """API endpoint for paginated AJAX results for trending keyword lists."""
    keywords = load_keywords()
    keyword = keyword.lower().strip()
    if keyword not in keywords:
        abort(404)
...
# ==============================================================
# Live (CT-confirmed) Domain Routes
#
# Lists are generated by scripts/generate_live_domains.py, which checks
# each day's trending keyword domains against a self-hosted Certificate
# Transparency API to flag ones that already have a live HTTPS cert.
# Mirrors the /trending routes above (same keyword categories).
# ==============================================================
@app.route("/live", defaults={"keyword": None})
@app.route("/live/<keyword>")
def live(keyword):
    keywords = load_keywords()
    ...
    if keyword:
        ...
        date = latest_date_with_live()
        if not date:
            abort(404)

        summary_data = load_json(JSON_FOLDER / "live.json") or {}
        counts = summary_data.get("keywords", {})

        file_path = LISTS_FOLDER / date / "live" / f"{keyword}.txt.gz"
        file_size = file_path.stat().st_size if file_path.exists() else 0
    ...
    data = load_json(JSON_FOLDER / "live.json")

    if not data:
        return jsonify({
            "error": "No live-domain statistics found",
            "keywords": {}
        }), 404
...
@app.route("/live/<keyword>/browse")
def browse_live(keyword):
    """API endpoint for paginated AJAX results for CT-confirmed live domains."""
    keywords = load_keywords()
    keyword = keyword.lower().strip()
    if keyword not in keywords:
        abort(404)
...
# Threat Match Routes
#
# Lists are generated by scripts/generate_threat_matches.py, which
# cross-references each day's new/all.txt.gz against public threat
# intel feeds (OpenPhish, URLhaus). Mirrors the /deleted pattern routes.
# ==============================================================
@app.route("/threats", defaults={"source": None})
@app.route("/threats/<source>")
def threats(source):
    if source:
        ...
        date = latest_date_with_threats()
        if not date:
            abort(404)

        summary_data = load_json(JSON_FOLDER / "threats.json") or {}
        source_counts = summary_data.get("sources", {})
        count = summary_data.get("total", 0) if source == "all" else source_counts.get(source, 0)

        file_path = LISTS_FOLDER / date / "threats" / f"{source}.txt.gz"
        file_size = file_path.stat().st_size if file_path.exists() else 0

        return render_template(
            "threats.html",
            source=source,
            source_label=THREAT_SOURCES[source],
            count=count,
            date=date,
            file_size=file_size,
    ...
    data = load_json(JSON_FOLDER / "threats.json")

    if not data:
        return jsonify({
            "error": "No threat match statistics found",
            "sources": {}
        }), 404
...
@app.route("/threats/<source>/browse")
def browse_threats(source):
    """API endpoint for paginated AJAX results for threat match lists."""
    source = source.lower().strip()
    if source not in THREAT_SOURCES:
        abort(404)
...
def _resolve_download_spec(spec):
    ...
    if spec.get("type") == "keyword":
        ...
        if keyword not in load_keywords():
            return None
        ...
        return file_path, f"trending-{keyword}-{date}.txt.gz", "application/gzip"

    if spec.get("type") == "live":
        keyword = spec.get("keyword", "")
        if keyword not in load_keywords():
            return None
        ...
        return file_path, f"live-{keyword}-{date}.txt.gz", "application/gzip"

    if spec.get("type") == "threat":
        ...
        return file_path, f"threats-{source}-{date}.txt.gz", "application/gzip"
...
def download_prepare():
    ...
    if dtype == "tld":
        ...
        keyword = str(payload.get("keyword", "")).lower().strip()
        if keyword not in load_keywords():
            abort(400)
        spec = {"type": "keyword", "keyword": keyword}
    elif dtype == "live":
        keyword = str(payload.get("keyword", "")).lower().strip()
        if keyword not in load_keywords():
            abort(400)
        spec = {"type": "live", "keyword": keyword}
    elif dtype == "threat":
        source = str(payload.get("source", "")).lower().strip()
        if source not in THREAT_SOURCES:
            abort(400)
        spec = {"type": "threat", "source": source}
    else:
        abort(400)
